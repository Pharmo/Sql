/* Femi Antwi's SQL Project
Introduction:
   This SQL project explores various queries on a 'superbowls' table, aiming to analyze and gain insights from 
   historical Super Bowl data. The dataset includes information about Super Bowl events, including dates, teams,
   scores, MVPs, and more.
*/
...
-- Query 1: Retrieve all columns from the 'superbowls' table
SELECT * FROM superbowls;

-- Query 2: Calculate the Average, Max, and Min values for Superbowl numeric columns
SELECT 
    AVG(Winner_pts) AS Avg_Winner_Points,
    MAX(Winner_pts) AS Max_Winner_Points,
    MIN(Winner_pts) AS Min_Winner_Points,
    AVG(Loser_pts) AS Avg_Loser_Points,
    MAX(Loser_pts) AS Max_Loser_Points,
    MIN(Loser_pts) AS Min_Loser_Points
FROM superbowls;

-- Query 3: Sample query using HAVING clause to filter results based on average winner points
SELECT Winner, AVG(Winner_pts) AS Avg_Winner_Points
FROM superbowls
GROUP BY Winner
HAVING AVG(Winner_pts) > 30;

-- Query 4: Sample query using CASE statement to categorize winners based on points
SELECT 
    Winner, Winner_Pts,
    CASE 
        WHEN Winner_pts < 14 THEN 'Low Scorer'
        WHEN Winner_pts >= 14 AND Winner_pts < 28 THEN 'Moderate Scorer'
        ELSE 'High Scorer'
    END AS Scoring_Category
FROM superbowls;

-- Query 5: Sample query using AND and OR combination in a WHERE clause
SELECT *
FROM SuperBowls
WHERE (Winner_pts > 35) OR (State = 'California' AND Winner_pts > 28);

-- Query 6: Retrieve details of the Super Bowl with the highest combined total points
SELECT Date, SB, Winner, Winner_pts, Loser, Loser_pts, (Winner_Pts + Loser_pts) AS Total_Points
FROM superbowls
ORDER BY (Winner_pts + Loser_pts) DESC
LIMIT 1;

-- Query 7: Find MVPs who won multiple Super Bowls
SELECT MVP, COUNT(Winner) AS SuperBowls_Won
FROM superbowls
GROUP BY MVP
HAVING COUNT(Winner) > 1;

-- Query 8: Find Super Bowls where the winning team's points were higher than the average points scored
SELECT *
FROM superbowls
WHERE Winner_pts > (SELECT AVG(Winner_pts) FROM superbowls);

-- Query 9: Calculate the total number of Super Bowls won by teams from each state
SELECT State, COUNT(Winner) AS Total_Wins
FROM superbowls
GROUP BY State
ORDER BY Total_Wins DESC;

-- Query 10: Retrieve details of the Super Bowl with the closest score
SELECT Date, SB, Winner, Winner_pts, Loser, Loser_pts, (Winner_Pts - Loser_Pts) AS closest_score
FROM superbowls
ORDER BY closest_score
LIMIT 1;
